...
class PalaceRepository {
  ...
  // NEW: Update an existing room's title
  Future<void> updateRoomTitle(int id, String newTitle) async {
    await (_db.update(_db.palaces)..where((t) => t.id.equals(id)))
        .write(PalacesCompanion(title: Value(newTitle)));
  }
  ...
  Future<void> saveMessage(int palaceId, String text, bool isUser) async {
    await _db.into(_db.chatMessages).insert(
      ChatMessagesCompanion.insert(palaceId: palaceId, messageText: text, isUser: isUser),
    );
  }
  ...
  Future<List<Edge>> getEdgesForPalace(int palaceId) {
    return (_db.select(_db.edges)..where((t) => t.palaceId.equals(palaceId))).get();
  }
  
  Future<List<Node>> getNodesForPalace(int palaceId) async {
    final edges = await getEdgesForPalace(palaceId);
    if (edges.isEmpty) return [];
    final nodeIds = edges.expand((e) => [e.sourceId, e.targetId]).toSet().toList();
  ...
  Future<void> updateNode(String id, String newLabel) async {
    await (_db.update(_db.nodes)..where((t) => t.id.equals(id))).write(NodesCompanion(label: Value(newLabel)));
  }
  Future<void> updateEdge(int id, String newLabel) async {
    await (_db.update(_db.edges)..where((t) => t.id.equals(id))).write(EdgesCompanion(label: Value(newLabel)));
  }
}
