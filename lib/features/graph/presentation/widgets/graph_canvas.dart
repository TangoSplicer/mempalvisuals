...
class _GraphCanvasState extends State<GraphCanvas> {
  ...
  void _runPhysicsSimulation() {
    ...
    for (var node in widget.nodes) {
      _positions[node.id] = center + Offset(random.nextDouble() * 200 - 100, random.nextDouble() * 200 - 100);
    }

    double k = 200.0; // Optimal distance between nodes
    double temp = 300.0; // MAXIMUM speed limit per iteration to prevent explosion
    ...
    for (int i = 0; i < 150; i++) {
      ...
      for (var node in widget.nodes) {
        ...
        if (dist > 0) {
          displacements[node.id] = displacements[node.id]! + (gravityDisp / dist) * (dist * 0.05);
        }
      }
