...
class _GraphCanvasState extends State<GraphCanvas> {
  ...
  // Custom Spring-Embedder Algorithm running locally (No jumping, allows dragging)
  void _runPhysicsSimulation() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      // 3. Gravity (Pull everything slightly to the absolute center so nothing shoots off)
      for (var node in widget.nodes) {
        var gravityDisp = center - _positions[node.id]!;
        displacements[node.id] = displacements[node.id]! + gravityDisp * 0.05;
      }
  ...
  Widget build(BuildContext context) {
    // Dynamically filter active data based on the Line of Enquiry (Session ID)
