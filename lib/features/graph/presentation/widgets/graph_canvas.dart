...
class _GraphCanvasState extends State<GraphCanvas> {
      ...
      for (var node in widget.nodes) {
        ...
        if (dist > 0) {
          displacements[node.id] =
              displacements[node.id]! + (gravityDisp / dist) * (dist * 0.02);
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    Set<String> lineageNodeIds =
        _focusedNodeId != null ? _getLineage(_focusedNodeId!, activeEdges) : {};
    ...
    return Stack(
      children: [
        ...
        if (activeNodes.isNotEmpty)
          GestureDetector(
            onTap: () => setState(
                () => _focusedNodeId = null), // Tap background to clear focus
            child: InteractiveViewer(
              transformationController: _transformationController,
              constrained: false,
              boundaryMargin: const EdgeInsets.all(
                  10000), // Massive margin to prevent clipping
              minScale: 0.02,
              maxScale: 5.0,
              child: SizedBox(
                width: _canvasSize,
                      ...
                      return Positioned(
                        ...
                        child: GestureDetector(
                          ...
                          child: AnimatedOpacity(
                            ...
                            opacity:
                                isFocused ? 1.0 : 0.15, // Dim unrelated nodes
                            child: Container(
