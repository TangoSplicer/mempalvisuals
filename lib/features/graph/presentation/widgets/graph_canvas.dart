...
class _GraphCanvasState extends State<GraphCanvas> {
  ...
  final double _canvasSize =
      40000.0; // MASSIVE canvas expansion to solve clipping
  ...
  void _runPhysicsSimulation() {
    ...
    for (var node in widget.nodes) {
      _positions[node.id] = center +
          Offset(
              random.nextDouble() * 800 - 400, random.nextDouble() * 800 - 400);
    }
      ...
      for (var node in widget.nodes) {
        ...
        if (dist > 0) {
          displacements[node.id] =
              displacements[node.id]! + (gravityDisp / dist) * (dist * 0.015);
        }
      }
  ...
  Set<String> _getLineage(String rootId, List<db.Edge> edges, int maxDepth) {
    ...
    for (int i = 0; i < maxDepth; i++) {
      ...
      for (var edge in edges) {
        if (currentLevel.contains(edge.sourceId) &&
            !visited.contains(edge.targetId)) {
          nextLevel.add(edge.targetId);
        }
        if (currentLevel.contains(edge.targetId) &&
  ...
  Widget build(BuildContext context) {
    ...
    Set<String> lineageNodeIds = _focusedNodeId != null
        ? _getLineage(_focusedNodeId!, activeEdges, _lineageDepth)
        : {};
          ...
          GestureDetector(
            ...
            child: InteractiveViewer(
              ...
              boundaryMargin: const EdgeInsets.all(
                  20000), // Massive margin to accommodate the 40000 canvas
              minScale: 0.01,
              maxScale: 5.0,
              child: SizedBox(
                width: _canvasSize,
                ...
                child: Stack(
                  children: [
                    ...
                    ...activeNodes.map((node) {
                      ...
                      final isFocused = _focusedNodeId == null ||
                          lineageNodeIds.contains(node.id);
                      ...
                      return Positioned(
                        ...
                        child: GestureDetector(
                          onTap: () => setState(() {
                            ...
                            _lineageDepth =
                                1; // Reset depth on new node selection
                          }),
                          onPanUpdate: (details) {
                            setState(() {
                              final scale = _transformationController.value
                                  .getMaxScaleOnAxis();
                              _positions[node.id] = _positions[node.id]! +
                                  (details.delta / scale);
                            });
                          },
                          child: AnimatedOpacity(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                boxShadow: [
                                  if (isMatch || _focusedNodeId == node.id)
                                    BoxShadow(
                              ...
                              child: Text(
                                ...
                                style: TextStyle(
                                    color:
                                        isMatch ? Colors.black : Colors.white,
                                    fontSize: 11,
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Slider(
                        ...
                        onChanged: (val) =>
                            setState(() => _lineageDepth = val.toInt()),
                      ),
                    ),
                    Text('$_lineageDepth',
                        style: const TextStyle(
...
class _EdgePainter extends CustomPainter {
  ...
  _EdgePainter(
      this.edges, this.positions, this.focusedNodeId, this.lineageNodeIds);
  ...
  void paint(Canvas canvas, Size size) {
    for (final edge in edges) {
      if (positions.containsKey(edge.sourceId) &&
        ...
        bool isFocusedEdge = focusedNodeId == null ||
            (lineageNodeIds.contains(edge.sourceId) &&
                lineageNodeIds.contains(edge.targetId));
